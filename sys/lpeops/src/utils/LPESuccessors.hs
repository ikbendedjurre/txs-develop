...
isPossibleSuccessor summand invariant candidate = do
    g <- doBlindSubst (lpeSmdEqs summand) (lpeSmdGuard candidate)
    Sat.couldBeSatisfiable (ValExpr.cstrAnd (Set.fromList [lpeSmdGuard summand, g])) invariant
-- isPossibleSuccessor
...
isDefiniteSuccessor summand invariant candidate = do
    g <- doBlindSubst (lpeSmdEqs summand) (lpeSmdGuard candidate)
    Sat.isTautology (ValExpr.cstrAnd (Set.fromList [lpeSmdGuard summand, g])) invariant
-- isDefiniteSuccessor
